%% Linear SVM on 3D point clouds of Oakland
...
% Class label definitions: veg, ground
 %classes = [1004, 1200];
 
 % Class label definitions: veg, wire
 %classes = [1004, 1100];
 
 % Class label definitions: wire, pole
 %classes = [1100, 1103];
 
 % Class label definitions: pole, ground
 %classes = [1103, 1200];
 
 % Class label definitions: ground, facade
 classes = [1400, 1200];
...
for iter = 1:1
    ...
    for t = 1:T1
       ...
       for i = 1:C
           if (0 > 1-(labels1(t,i)*w(:,i)'*f_training(t,:)'))
               grad = 2*w(:,i)'*lambda;
           else
            grad = 2*lambda*w(:,i)' - (labels1(t,i)*f_training(t,:));
           end
            alpha = 1/(sqrt(T1));
            w(:,i) = w(:,i) - alpha*grad';
...
for t = 1:T2
    ...
    if (labels2(t,1)==1 || labels2(t,2)==1)
